    ...
    <div class="mx-auto w-full max-w-3xl space-y-6">
        <div class="hero-panel p-6">
            <div class="hero-panel-content flex flex-col gap-2 text-white">
                ...
                <h1 class="text-3xl font-bold">Editar ubicación</h1>
                <p class="text-sm text-white/80">{{ $location->name }}</p>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <form method="POST" action="{{ route('food.locations.update', $location) }}" class="space-y-6 p-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="{{ $label }}">Nombre de la ubicación *</label>
                    <input id="name" name="name" value="{{ old('name', $location->name) }}" required class="{{ $input }}" />
                </div>

                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        <label for="color" class="{{ $label }}">Color identificador</label>
                        <input id="color" name="color" type="color" value="{{ old('color', $location->color ?? '#22c55e') }}" class="mt-1 h-11 w-full rounded-xl border border-gray-200 bg-white px-3 text-sm text-gray-900 shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" />
                    </div>
                    <div>
                        <label for="sort_order" class="{{ $label }}">Orden de visualización</label>
                        <input id="sort_order" name="sort_order" type="number" min="0" value="{{ old('sort_order', $location->sort_order) }}" class="{{ $input }}" />
                    </div>
                </div>

                <div class="flex items-center gap-3">
                    <input type="checkbox" id="is_default" name="is_default" value="1" class="h-4 w-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500" @checked(old('is_default', $location->is_default))>
                ...
                <div class="flex items-center justify-end gap-3">
                    <a href="{{ route('food.locations.show', $location) }}" class="{{ $btnSecondary }}">Cancelar</a>
                    <button type="submit" class="{{ $btnPrimary }}">Actualizar ubicación</button>
                </div>
            </form>
        </div>
    </div>
